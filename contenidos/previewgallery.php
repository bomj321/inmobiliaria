...
?>
	<script type="text/javascript">
            ...
            $("#multiFiles").change(function(e){
                    ...
                    $.ajax({
                        url: '<?php echo DIR;?>contenidos/uploads-servicios', 
                        dataType: 'text', 
                        cache: false,
                        contentType: false,
                        processData: false,
    ...
    <div class="uk-modal-dialog uk-margin-auto-vertical">
       <span id="modclose2"> <button class="uk-modal-close-default" type="button" uk-close></button></span>
        <div class="uk-modal-header">
			...
			<h5 class="uk-modal-title "><span uk-icon="icon:image;ratio:1.2;" class="icon-margin"></span> Editar galería de imagenes de la propiedad: <strong class="yellow"><?php echo $refid.'V'?></strong></h5>
				</div>
				
			</div>
        </div>
<?php $stmt6 = $db->prepare("SELECT * FROM image_services where ref='$refid' order by orden asc");
...
<?php $idimagen=$row6['id'] ?>
   <li id="<?php echo $row6['orden']?>" value="<?php echo $row6['id'];?>">
		...
		<div class="image uk-card">
			...
			<div class="uk-inline-clip uk-transition-toggle uk-light covergallery" tabindex="0"><img src="<?php echo '../../services/'.$row6['small']?>">
				<div class="uk-position-top-right" style="top:10px !important; right:10px !important">
	                <span class="uk-transition-slide-top-small delete" uk-icon="icon: trash;" onclick="deleteimg(<?php echo $ordenar?>,<?php echo $row6['id']?>)" ></span>
	            </div>
           </div>
...
function deleteimg(orden2,idimagen) {
    ...
    $.ajax({
	type: "POST",
	url: "<?php echo DIR;?>contenidos/deleteimg",
...
function manageGallery(param3) {
	...
	if (confirm("¿Quiere cerrar sin guardar? Se perderán todos los cambios.")) {
         $.ajax({
	type: "POST",
	url: "<?php echo DIR;?>contenidos/managegallery",
...
function manageGallery2(param3) {
	...
	success: function(data){
		...
		$( "#<?php echo $divid; ?>" ).load(" #<?php echo $divid; ?>");
		//$("#container_todo").load(" #container_todo");
