...
<div class="uk-modal-dialog uk-margin-auto-vertical" style="width:70%;">
        <button class="uk-modal-close-default" onclick="closeEdit4()" type="button" uk-close></button>
		...
		<form class="uk-form-stacked" id="distanciaform2">	
<?php $refauto = $db->prepare("SELECT id FROM distancias_properties ORDER BY ID DESC LIMIT 1");
...
$ref_ok=$refauto1['id']+1;}?>
	...
	<div class="uk-width-1-1" style="margin-top:10px">
		...
		<hr class="uk-article-divider">
					<div class=" uk-grid-medium uk-child-width-auto uk-grid ">
            <label style="cursor:pointer"><input class="uk-checkbox" name="distanciaActivo" type="checkbox" checked value="si">&nbsp; ¿Activar distancia/entorno?</label>
			</div>	
			</div>   
        ...
        <div class="uk-modal-footer uk-text-right">
            ...
            <button onclick="closeEdit4()" class="uk-button uk-button-default" type="button">Cancelar</button>
            <button onclick="saveData4('1')" class="uk-button uk-button-primary" type="button"><strong>Guardar distancia <span uk-icon="icon:push;ratio:0.95" class="icon-margin"></span></strong></button>
        </div>
		
			</form>
    </div>
<script type="text/javascript">
function saveData4(param) {
	if (confirm("¿Confirma que desea guardar la nueva distancia/entorno?")) {
			...
			$.ajax({
                ...
                data : $("#distanciaform2").serialize(), // post data || get data
