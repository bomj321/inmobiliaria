...
$orden2=$_POST["orden"];
$tipo=$_POST["tipo"];
$delete = "DELETE from image_properties_rentals where id='$refid' and orden='$orden2'";
$stmt44 = $db->prepare("SELECT original,full,medium,small FROM image_properties_rentals WHERE id='$refid' and orden='$orden2'");
$stmt44->execute();
...
$db->exec($delete);
$stmt6666 = $db->prepare("SELECT * FROM image_properties_rentals where  ref='$ref3' order by orden asc");
$stmt6666->setFetchMode(PDO::FETCH_ASSOC);
$stmt6666->execute();	
$orden666=1;
while ($row6666 = $stmt6666->fetch()){
$value666=$row6666['id'];
$update226 = "UPDATE image_properties_rentals SET orden='$orden666' where image_properties_rentals.id='$value666'";
$db->exec($update226);	
$orden666++;	
}	
?>