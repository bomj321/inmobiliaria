...
$stmt7 = $db->prepare("SELECT MAX(orden) as maxorden FROM image_properties where ref='$ref'");
$stmt7->execute();
$row7 = $stmt7->fetch(PDO::FETCH_ASSOC);
$orden=$row7['maxorden'];
$orden1=$orden1+1;
if ($tipo=="cancelar"){
$stmt6 = $db->prepare("SELECT * FROM image_properties where ref='$ref' and orden between '$orden1' and '$orden' ");
$stmt6->setFetchMode(PDO::FETCH_ASSOC);
$stmt6->execute();	
while ($row6 = $stmt6->fetch()){
$orden2=$row6['orden'];
$delete = "DELETE from image_properties where ref='$ref' and orden='$orden2'";
$stmt44 = $db->prepare("SELECT original,full,medium,small FROM image_properties WHERE ref='$ref' and orden='$orden2'");
$stmt44->execute();
...
$update23 = "UPDATE image_properties SET caption='$captionselect2[$i]' where image_properties.id='$value2'";
$db->exec($update23);	
...
$update22 = "UPDATE image_properties SET orden='$orden' where image_properties.id='$value'";
$db->exec($update22);	
