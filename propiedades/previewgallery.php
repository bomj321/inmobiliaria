...
$stmt = $db->prepare("SELECT * FROM properties WHERE ID='$refid'");
...
?>
	...
	<script type="text/javascript">
            ...
            $("#multiFiles").change(function(e){
                    ...
                    $.ajax({
                        url: '<?php echo DIR;?>propiedades/uploads', 
                        dataType: 'text', 
                        cache: false,
                        contentType: false,
                        processData: false,
...
<?php 	
	$stmt6 = $db->prepare("SELECT * FROM image_properties where ref='$refid' order by orden asc");
...
<?php $idimagen=$row6['id'] ?>
   <li id="<?php echo $row6['orden']?>" value="<?php echo $row6['id'];?>">
		<div class="image uk-card">
				<div class="uk-inline-clip uk-transition-toggle uk-light covergallery" tabindex="0"><img src="<?php echo $row6['small']?>">
             ...
             <select placeholder="-Seleccionar-" onchange=""  class="select-gallery form-control" name="<?php echo $row6['id'];?>">
...
function deleteimg(orden2,idimagen) {
    ...
    $.ajax({
	type: "POST",
	url: "<?php echo DIR;?>propiedades/deleteimg",
...
function manageGallery(param3) {
	...
	if (confirm("¿Quiere cerrar sin guardar? Se perderán todos los cambios.")) {
         $.ajax({
	type: "POST",
	url: "<?php echo DIR;?>propiedades/managegallery",
