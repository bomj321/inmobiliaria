...
/*ACTUALIZACION 31 DATOS*/

mysqli_set_charset($connection, "utf8");
    $sql="UPDATE properties SET active=?,SellerID=?,NameOwner=?, propLocation= ?, propStatus= ?, propFeatured= ?, propNameES= ?, propNameEN= ?, propNameDE= ?,propAddress= ?, propLinkMap=?, mostrarDireccion=?,propType=?,propPrice=?,propDescripES=?,propDescripEN=?,propDescripDE=?,propHouseM2=?,propTerraceM2=?,propLandM2=?,propTotalM2=?,propBedSingle=?,propBedDouble=?,propBathroom=?,propToilet=?,esLujo=?,esNueva=?,clasifEnergia=?,slider=?,propNotesPrivate=?  WHERE ID=?";
    $resultado=mysqli_prepare($connection, $sql);
    mysqli_stmt_bind_param($resultado, "sssssssssssssssssssssssssssssss",$active,$propietario,$name_owner,$location,$estado1,$destacada,$tituloES,$tituloEN,$tituloDE,$direccion,$linkmap,$mostrarDireccion,$tipoProp2,$precioVenta,$descripES,$descripEN,$descripDE,$supUtil,$supTerraza,$supTerreno,$supTotal,$habSimple,$habDoble,$banos_extra,$aseos,$lujo,$nueva,$clasifEnergia,$portada,$notas,$id_venta);
