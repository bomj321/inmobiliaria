...
$exists = array();
$refid2=$_POST["refid"];
$stmt7 = $db->prepare("SELECT MAX(orden) as maxorden FROM image_properties where ref='$refid2'");
$stmt7->execute();
...
?>
<?php 
$imginsert = "INSERT INTO image_properties (ref,original,full,medium,small,alt,orden,caption) VALUES ('".$refid2."','".$filename."','".$full."','".$medium."','".$small."','".$alt."','".$orden."','".$caption."')";
$db->exec($imginsert);	
$stmt77 = $db->prepare("SELECT id FROM image_properties where orden='$orden' and ref='$refid2'");
$stmt77->execute();
$row77 = $stmt77->fetch(PDO::FETCH_ASSOC);
?>
<li id="<?php echo $orden?>" value="<?php echo $row77['id'];?>">

		<div class="image uk-card"><div class="uk-inline-clip uk-transition-toggle uk-light covergallery" tabindex="0"><?php echo '<img src="../../uploads/' . $filename . '">';?>
